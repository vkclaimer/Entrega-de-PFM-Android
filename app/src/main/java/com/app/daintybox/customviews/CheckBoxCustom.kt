...
class CheckBoxCustom: AppCompatCheckBox {
    ...
    fun init_font(attrs: AttributeSet?, context: Context?) {
        if (attrs != null) {
            val a = getContext().obtainStyledAttributes(attrs, R.styleable.CheckBoxCustom)
            val fontName = a.getString(R.styleable.CheckBoxCustom_fontName)
            if (fontName != null) {
                when (fontName) {
                    "semibold" -> {
